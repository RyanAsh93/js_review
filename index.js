...
function filterOverForty() {
  const { employees } = state 
  const employessOverForty = employees.filter((emp) => emp.employee_age > 40)
  state.employees = employessOverForty
  render()
}

function reset() {
  const { originalEmployees } = state
...
function getEmployees() {
  ...
  axios
    ...
    .then((response) => {
      ...
      state.originalEmployees = response.data.data;
